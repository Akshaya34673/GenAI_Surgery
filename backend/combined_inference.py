# --------------------------------------------------------------
#  combined_inference.py  (FULLY UPDATED & READY TO PASTE)
# --------------------------------------------------------------
from flask import Blueprint, request, jsonify, send_from_directory
from werkzeug.utils import secure_filename
import os, glob, cv2, torch, jwt as pyjwt, subprocess, shutil
...
# ----------------------------------------------------------------------
#  Generate full-duration annotated video (FFmpeg Full Path)
# ----------------------------------------------------------------------
def generate_annotated_video(input_video_path, output_path,
    ...
    # Temporary folder for annotated frames
    temp_dir = os.path.join(UPLOAD_FOLDER, "temp_annotated_frames")
    ...
    while True:
        ...
        png_name = f"frame_{frame_idx:06d}.png"
    ...
    cap.release()
    print(f"All {frame_idx} annotated frames saved to {temp_dir}")

    # ------------------------------------------------------------------
    #  Stitch PNGs to MP4 using YOUR FFmpeg (full path)
    # ------------------------------------------------------------------
    FFMPEG_PATH = r"C:\ffmpeg\ffmpeg-8.0-essentials_build\bin\ffmpeg.exe"
    if not os.path.isfile(FFMPEG_PATH):
        raise FileNotFoundError(f"FFmpeg not found at:\n  {FFMPEG_PATH}\n"
    ...
    # Fast-start for web
    fast_path = output_path.replace('.mp4', '_fast.mp4')
    cmd_fast = [
        FFMPEG_PATH, '-i', output_path,
        '-c', 'copy', '-movflags', '+faststart',
        '-y', fast_path
    ]
    subprocess.run(cmd_fast, check=True, capture_output=True)
    ...
    print(f"Annotated video ready: {output_path}")
    return output_path
...
def analyze_surgical_video_route():
    ...
    try:
        # ----- Global inference -----
        phase_step_res = run_phase_step_inference(input_path, PHASE_STEP_MODEL)
        atomic_res     = run_atomic_actions_inference(input_path, ATOMIC_ACTIONS_MODEL)
        ...
        # ----- Save to DB with relative paths -----
        rel_input = f"Uploads/{safe_name}"
        rel_output = f"Uploads/annotated_{safe_name}"
